...
//              Define Client
//////////////////////////////////////////////
val all_data = consult("claro","2017-09-15 00:00:00","2017-09-16 00:00:00")
...
val resultSet3 = resultSet2.withColumn("date_time",date_format($"date_time","yyyy-MM-dd HH:mm:ss"))

val resultSet3_ordered = resultSet3.orderBy("date_time")
val maxmin = resultSet3_ordered.withColumn("datamax",max($"date_time").over(Window.partitionBy("visitId","mac_address","date_diaria"))).withColumn("datamin",min($"date_time").over(Window.partitionBy("visitId","mac_address","date_diaria")))
val timediff = maxmin.withColumn("timediff",(unix_timestamp($"datamax")-unix_timestamp($"datamin"))/60)
val general_mean = timediff.groupBy("id_sensor").agg(mean($"rssi").alias("signal"),countDistinct($"mac_address").alias("count"), mean($"timediff").alias("time"))
val data_per_sensor = timediff.groupBy("id_sensor","date_diaria").agg(mean($"rssi").alias("signal"),countDistinct($"mac_address").alias("count"), mean($"timediff").alias("time"))

//resultSet3_ordered.rdd.map(x=>x.mkString(",")).coalesce(1,true).saveAsTextFile("/home/centos/" + clientg + datetime_inig.split(" ")(0) + "_" + datetime_finalg.split(" ")(0))
general_mean.rdd.map(x=>x.mkString(",")).coalesce(1,true).saveAsTextFile("/home/centos/general_mean_" + clientg + datetime_inig.split(" ")(0) + "_" + datetime_finalg.split(" ")(0))
data_per_sensor.rdd.map(x=>x.mkString(",")).coalesce(1,true).saveAsTextFile("/home/centos/data_per_sensor_" + clientg + datetime_inig.split(" ")(0) + "_" + datetime_finalg.split(" ")(0))
